        ...
        try:
            ...
            if audio.shape[0]>0:
                if (np.abs(audio)).var()<3.0:
                    print("Error, low var: ",np.abs(audio).var()," combi:",combi)
                    low_var+=1
                    if low_var>=5:
                        break
                    continue
            ...
            continue
    if low_var>=5:
        raise Exception("Low var exception, skipping instrument")
    return audio
...
def room_sound_simulation(data):
    ...
    ceiling_materials = ['ceiling_plasterboard','ceiling_fissured_tile','ceiling_perforated_gypsum_board','ceiling_melamine_foam','wood_1.6cm','plywood_thin','wood_16mm',
                        'rough_concrete','ceramic_tiles','hard_surface','rough_concrete']
    # Raum erstellen
    room = pra.ShoeBox([width, length, height],fs=fs,ray_tracing=True, air_absorption=True,
    ...
    if with_drums:
        ...
        for instrument in midi_data.instruments:
            ...
            if len(instrument.notes)>0:
                ...
                if mv > maxv:
                    maxv = mv
        audio = midi_copy.fluidsynth(fs=fs, sf2_path=getRandomSF2Path(soundfonts_path))
...
parser.add_argument("--sampling_rate", type=int,default=16000, help="")
